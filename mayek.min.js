...
if((scrollPos == ($(document).height()- window.innerHeight*2)) && (window.innerWidth <768)){
        $('.scroll').addClass('offdown');
      }
      else{
        $('.scroll').removeClass('offdown');
   ...
   if(x4 == ($('.tall-container4').height()-window.innerHeight)){
    ...
    $(".site-header").css("visibility",'visible');
	    ...
	    if(x4 == ($('.tall-container4').height()-window.innerHeight*2)){
    $(".site-header").css("visibility",'hidden');
   }
   else{
    $(".site-header").css("visibility",'visible');
         ...
         if ($(".bars.b3").height()<=window.innerHeight){
          ...
          $(".letter").css("left", Math.round(((x2-(window.innerHeight*2))-($(".letter-container").width()))/10));
...
ctx.fillStyle = '#02563ed4';
ctx.beginPath();
ctx.moveTo(Math.round(window.innerWidth/1.5), 0);
ctx.lineTo(window.innerWidth, 0);
ctx.lineTo(window.innerWidth, window.innerHeight);
ctx.lineTo(Math.round(window.innerWidth/3), window.innerHeight);
ctx.closePath();
ctx.fill();
//ctx.clip();
        ctx.drawImage(img, dx, dy, wx, wh);
...
function line1(){
    ...
    var anime =  setInterval(function() {
       ...
       // ctx.save(); 
        ...
        console.log(Math.floor(startX + (endX - startX) * amm))
          console.log(Math.floor( startY + (endY - startY) * amm))
        //console.log(startY + (endY - startY) * .5)
        ctx.lineTo(startX + (endX - startX) * amount , startY + (endY - startY) * amount);
        console.log(startX + (endX - startX) * amount )
        console.log(startY + (endY - startY) * amount)
        ctx.stroke();
        if(amount <= 0){
    clearInterval(anime);
    //ctx.restore(); 
...
//right rect
	...
	var movex1 = Math.floor(window.innerWidth/1.5) + movex-1;
	 var movex2 = window.innerWidth + movex;
		var movex3 = window.innerWidth + movex;
		var movex4 = Math.floor(window.innerWidth/3) + movex-1;
